...
- **贴边吸附** — 拖到屏幕边缘收起为双胶囊迷你条（两根进度条并排），悬停展开、离开约 0.5 秒自动收回；贴边状态跨重启记忆
- **悬停气泡** — 收回迷你条的瞬间弹出数字气泡，两行配额百分比 + 重置倒计时，3 秒自动消失
- **右键菜单** — 刷新、定位、主题切换、设置、退出
...
- **迷你条布局转置重制**：右/左缘收起态改为 32×80 双胶囊竖条并排（MCP + 5h），顶缘 126×32 双横条；填充自底向上/自左向右生长，条序与展开卡片一致（MCP 恒为阅读首位）
- **悬停气泡修复**：展开卡片不再清空气泡文本，收回瞬间强制弹出数字气泡（3 秒自动消失）——此前"迷你条悬停气泡"因 Win32 tooltip 重挂机制实际永远不显示
- 收回瞬间鼠标仍在条上时立即重新展开（hover 优先），消除交互死区
- 展开卡片与迷你条加 1px 内描边（浅色 #C6CDDC），半像素内切杜绝 Region 切边锯齿
- 修复部分机器（系统最小窗口宽度被放大到 178px 级）无边框迷你条被强制撑宽的畸形
- 修复贴边状态恢复漏掉右缘（DockStyle=4 持久化值不在旧范围检查内）
...
- **请求超时**：10 秒
- **重试策略**：请求失败后指数退避自动重试（间隔逐次翻倍，封顶 30 分钟），恢复网络后自动回到正常轮询节奏；也可随时手动刷新
